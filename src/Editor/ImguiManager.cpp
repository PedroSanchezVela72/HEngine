...
{
    if (mFontTex) {
        mFontTex.reset(); // Limpiar la textura de la fuente
    }
...
void ImguiManager::renderQueueEnded(uint8 queueGroupId, const String& invocation,bool& repeatThisInvocation)
{
    ...
    for (int i = 0; i < draw_data->CmdListsCount; ++i)
    {
        ...
        for (int j = 0; j < draw_list->CmdBuffer.Size; ++j)
        {
            ...
            renderSys->setScissorTest(true, Rect(scLeft, scTop, scRight, scBottom));
...
ImFont* ImguiManager::addFont(const String& name, float sizePixel, const String& group )
{
    ...
    if (sizePixel == 0) {
        sizePixel = font->getTrueTypeSize();
    }
    ImFontConfig cfg;
    strncpy_s(cfg.Name, name.c_str(), 40);
    return io.Fonts->AddFontFromMemoryTTF(ttfchunk.getPtr(), ttfchunk.size(),
        sizePixel, &cfg, cprangePtr);
#else
    OGRE_EXCEPT(Exception::ERR_INVALID_CALL, "Ogre Overlay Component required");
    return NULL;
#endif
}
void ImguiManager::addFontTexture()
{
    createFontTexture();
    createMaterial();
}
void ImguiManager::destroyFontTexture()
{
    if (mFontTex)
    {
        // Desregistra la textura del TextureManager
        TextureManager::getSingleton().remove(
            mFontTex->getName(),
            mFontTex->getGroup()  // normalmente INTERNAL_RESOURCE_GROUP_NAME
        );
        mFontTex.reset();
    }

    // También limpia el atlas de ImGui para poder reconstruirlo sin residuos
    ImGuiIO& io = ImGui::GetIO();
    io.Fonts->ClearInputData();  // libera los buffers de TTF que quedaron
    io.Fonts->ClearTexData();    // elimina cualquier data de textura previa
    io.Fonts->Clear();  // libera todos los ImFont*
    // 3) (Opcional) Remueve el material para forzar su recreación
    if (MaterialManager::getSingleton().resourceExists("imgui/material"))
        MaterialManager::getSingleton().remove(
            "imgui/material",
            ResourceGroupManager::INTERNAL_RESOURCE_GROUP_NAME
        );
    mRenderable.mMaterial.reset();
}
